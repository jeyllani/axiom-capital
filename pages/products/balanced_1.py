...
# --- Marketing Section ---
st.title("⚖️ Global Core Balanced")
st.markdown("<h3 style='color: #94a3b8; font-weight: 400; margin-top: -10px;'>Optimal Risk-Adjusted Returns</h3>", unsafe_allow_html=True)
...
with col_m1:
    ...
    <div style="padding-right: 20px;">
        <p>
            The <b>Global Core Balanced</b> strategy is the flagship solution for investors seeking the highest possible return per unit of risk. 
            By mathematically maximizing the Sharpe Ratio, the portfolio allocates capital to assets that offer the best risk-reward trade-off.
        </p>
        <p>
            This strategy dynamically adapts to market conditions, increasing exposure to high-performing assets while maintaining a disciplined risk management framework.
        </p>
        <ul style="list-style-type: none; padding-left: 0;">
            <li style="margin-bottom: 10px;">✅ <b>Efficiency</b>: Targets the Tangency Portfolio on the Efficient Frontier.</li>
            <li style="margin-bottom: 10px;">✅ <b>Performance</b>: Historically superior risk-adjusted returns compared to benchmarks.</li>
            <li>✅ <b>Adaptability</b>: Rebalances monthly to capture evolving market trends.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    
    st.info("💡 **Investment Profile**: Balanced investors seeking growth with a focus on efficiency. Horizon: 5-7 Years.")

with col_m2:
    st.markdown("""
    <div class="marketing-card">
        <h4>Key Characteristics</h4>
        <div class="feature-item"><span class="feature-icon">📐</span> Max Sharpe Ratio</div>
        <div class="feature-item"><span class="feature-icon">⚖️</span> Optimal Trade-off</div>
        <div class="feature-item"><span class="feature-icon">🔄</span> Monthly Rebalancing</div>
        <div class="feature-item"><span class="feature-icon">🌟</span> Flagship Core</div>
    </div>
    """, unsafe_allow_html=True)
...
# --- Action Section ---
if 'show_results_balanced_1' not in st.session_state:
    st.session_state.show_results_balanced_1 = False

col_btn1, col_btn2, col_btn3 = st.columns([1, 2, 1])
with col_btn2:
    if st.button("🚀 Simulate Historical Performance (2010-2024)", use_container_width=True, type="primary"):
        st.session_state.show_results_balanced_1 = True

if st.session_state.show_results_balanced_1:
    st.markdown("---")
    
    # Run Strategy (Cached in Session State)
    if 'strategy_results_balanced_1' not in st.session_state:
        with st.spinner("🔄 Optimizing Portfolio & Running Backtest..."):
            st.session_state.strategy_results_balanced_1 = run_strategy("Balanced Sharpe")
    
    results = st.session_state.strategy_results_balanced_1
    ...
    with tab1:
        ...
        fig.add_trace(go.Scatter(x=cum_ret_series.index, y=cum_ret_series, name='Balanced Strategy', 
