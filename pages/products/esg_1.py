...
<style>
    ...
    .marketing-card h4 {
        color: #10b981; /* Emerald-500 for ESG */
        margin-bottom: 16px;
        font-weight: 600;
    }
    .feature-item {
...
# --- Marketing Section ---
st.title("🌱 Global Sustainable Future")
st.markdown("<h3 style='color: #94a3b8; font-weight: 400; margin-top: -10px;'>Impact Investing with Purpose</h3>", unsafe_allow_html=True)
...
with col_m1:
    ...
    <div style="padding-right: 20px;">
        <p>
            The <b>Global Sustainable Future</b> strategy allows you to align your investments with your values without compromising on performance. 
            It strictly excludes high-carbon sectors (Energy, Utilities, Materials) while optimizing for the highest risk-adjusted returns within a responsible universe.
        </p>
        <p>
            This portfolio is designed for the modern investor who believes that long-term value creation is inextricably linked to sustainable business practices.
        </p>
        <ul style="list-style-type: none; padding-left: 0;">
            <li style="margin-bottom: 10px;">✅ <b>Zero Carbon Exposure</b>: Complete exclusion of fossil fuel and resource-intensive sectors.</li>
            <li style="margin-bottom: 10px;">✅ <b>Efficient Growth</b>: Uses Sharpe Ratio optimization to find the best sustainable assets.</li>
            <li>✅ <b>Future-Proof</b>: Positioned for a transition to a low-carbon economy.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    
    st.info("💡 **Investment Profile**: Conscious investors seeking competitive returns through a responsible lens. Horizon: 5-7 Years.")

with col_m2:
    st.markdown("""
    <div class="marketing-card">
        <h4>Key Characteristics</h4>
        <div class="feature-item"><span class="feature-icon">🌱</span> ESG Screened</div>
        <div class="feature-item"><span class="feature-icon">🚫</span> Fossil Fuel Free</div>
        <div class="feature-item"><span class="feature-icon">🔄</span> Monthly Rebalancing</div>
        <div class="feature-item"><span class="feature-icon">🌍</span> Global Impact</div>
    </div>
    """, unsafe_allow_html=True)
...
# --- Action Section ---
if 'show_results_esg_1' not in st.session_state:
    st.session_state.show_results_esg_1 = False

col_btn1, col_btn2, col_btn3 = st.columns([1, 2, 1])
with col_btn2:
    if st.button("🚀 Simulate Historical Performance (2010-2024)", use_container_width=True, type="primary"):
        st.session_state.show_results_esg_1 = True

if st.session_state.show_results_esg_1:
    st.markdown("---")
    
    # Run Strategy (Cached in Session State)
    if 'strategy_results_esg' not in st.session_state:
        with st.spinner("🔄 Optimizing Portfolio & Running Backtest..."):
            st.session_state.strategy_results_esg = run_strategy("ESG Impact")
    
    results = st.session_state.strategy_results_esg
    ...
    with tab1:
        ...
        # Institutional Green Theme for ESG
        fig.add_trace(go.Scatter(x=cum_ret_series.index, y=cum_ret_series, name='ESG Strategy', 
                                 line=dict(color='#10b981', width=2)), row=1, col=1) # Emerald-500
    ...
    with tab4:
        ...
        with col_sim1:
            ...
            st.markdown(f"""
            <div style="margin-top: 20px; padding: 15px; background: rgba(16, 185, 129, 0.1); border-radius: 8px; border: 1px solid #10b981;">
                <h5 style="color: #10b981; margin:0;">Historical Result (2010-2024)</h5>
        ...
        with col_sim2:
            ...
            for path in paths[:50]: # Show first 50 paths
                fig_mc.add_trace(go.Scatter(x=time_axis, y=path, mode='lines', 
                                          line=dict(color='rgba(16, 185, 129, 0.1)', width=1), showlegend=False))
