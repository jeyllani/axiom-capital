...
# --- Marketing Section ---
st.title("⚖️ Global Risk Parity")
st.markdown("<h3 style='color: #94a3b8; font-weight: 400; margin-top: -10px;'>Equal Risk Contribution</h3>", unsafe_allow_html=True)
...
with col_m1:
    ...
    <div style="padding-right: 20px;">
        <p>
            The <b>Global Risk Parity</b> strategy represents a paradigm shift in portfolio construction. 
            Instead of allocating capital based on dollar amounts, it allocates based on risk. The goal is to ensure that every asset contributes equally to the portfolio's total volatility.
        </p>
        <p>
            This approach creates a truly diversified portfolio that is robust across different economic environments, avoiding the concentration risk typical of traditional 60/40 portfolios.
        </p>
        <ul style="list-style-type: none; padding-left: 0;">
            <li style="margin-bottom: 10px;">✅ <b>True Diversification</b>: Risk is spread evenly, not just capital.</li>
            <li style="margin-bottom: 10px;">✅ <b>All-Weather</b>: Designed to perform in inflation, deflation, growth, and recession.</li>
            <li>✅ <b>Stability</b>: Lower tail risk due to lack of risk concentration.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    
    st.info("💡 **Investment Profile**: Sophisticated investors seeking structural diversification and consistent risk-adjusted returns. Horizon: 5+ Years.")

with col_m2:
    st.markdown("""
    <div class="marketing-card">
        <h4>Key Characteristics</h4>
        <div class="feature-item"><span class="feature-icon">🧩</span> Risk Parity (ERC)</div>
        <div class="feature-item"><span class="feature-icon">⚖️</span> Equal Risk Contribution</div>
        <div class="feature-item"><span class="feature-icon">🔄</span> Monthly Rebalancing</div>
        <div class="feature-item"><span class="feature-icon">🌧️</span> All-Weather Design</div>
    </div>
    """, unsafe_allow_html=True)
...
# --- Action Section ---
if 'show_results_balanced_3' not in st.session_state:
    st.session_state.show_results_balanced_3 = False

col_btn1, col_btn2, col_btn3 = st.columns([1, 2, 1])
with col_btn2:
    if st.button("🚀 Simulate Historical Performance (2010-2024)", use_container_width=True, type="primary"):
        st.session_state.show_results_balanced_3 = True

if st.session_state.show_results_balanced_3:
    st.markdown("---")
    
    # Run Strategy (Cached in Session State)
    if 'strategy_results_balanced_3' not in st.session_state:
        with st.spinner("🔄 Optimizing Portfolio & Running Backtest..."):
            st.session_state.strategy_results_balanced_3 = run_strategy("Balanced RiskParity")
    
    results = st.session_state.strategy_results_balanced_3
    ...
    with tab1:
        ...
        fig.add_trace(go.Scatter(x=cum_ret_series.index, y=cum_ret_series, name='Risk Parity Strategy', 
