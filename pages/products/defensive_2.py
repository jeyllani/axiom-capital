...
# --- Marketing Section ---
st.title("🛡️ Global Conservative Yield")
st.markdown("<h3 style='color: #94a3b8; font-weight: 400; margin-top: -10px;'>Risk-Adjusted Income Focus</h3>", unsafe_allow_html=True)
...
with col_m1:
    ...
    <div style="padding-right: 20px;">
        <p>
            The <b>Global Conservative Yield</b> strategy is designed to optimize the trade-off between risk and return. 
            Utilizing a high risk aversion parameter (Utility Maximization), it seeks to generate superior risk-adjusted returns compared to pure defensive strategies.
        </p>
        <p>
            This approach intelligently balances safety with growth potential, making it suitable for investors who want protection but are willing to accept marginal volatility for better yield.
        </p>
        <ul style="list-style-type: none; padding-left: 0;">
            <li style="margin-bottom: 10px;">✅ <b>Efficient Frontier Positioning</b>: Targets the optimal "sweet spot" for conservative investors.</li>
            <li style="margin-bottom: 10px;">✅ <b>Enhanced Yield</b>: Slightly higher return potential than pure Minimum Variance.</li>
            <li>✅ <b>Controlled Risk</b>: Strict constraints to limit downside exposure.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    
    st.info("💡 **Investment Profile**: Conservative investors seeking a balance of safety and income. Horizon: 3-5 Years.")
...
with col_m2:
    ...
    <div class="marketing-card">
        ...
        <div class="feature-item"><span class="feature-icon">🛡️</span> High Risk Aversion</div>
        <div class="feature-item"><span class="feature-icon">🔄</span> Monthly Rebalancing</div>
        <div class="feature-item"><span class="feature-icon">📈</span> Risk-Adjusted Growth</div>
    </div>
    """, unsafe_allow_html=True)
...
# --- Action Section ---
if 'show_results_defensive_2' not in st.session_state:
    st.session_state.show_results_defensive_2 = False

col_btn1, col_btn2, col_btn3 = st.columns([1, 2, 1])
with col_btn2:
    if st.button("🚀 Simulate Historical Performance (2010-2024)", use_container_width=True, type="primary"):
        st.session_state.show_results_defensive_2 = True

if st.session_state.show_results_defensive_2:
    st.markdown("---")
    
    # Run Strategy (Cached in Session State)
    # Note: We use a different cache key for this strategy to avoid conflict
    if 'strategy_results_conservative' not in st.session_state:
        with st.spinner("🔄 Optimizing Portfolio & Running Backtest..."):
            st.session_state.strategy_results_conservative = run_strategy("Defensive Conservative")
    
    results = st.session_state.strategy_results_conservative
    ...
    with tab1:
        ...
        fig.add_trace(go.Scatter(x=cum_ret_series.index, y=cum_ret_series, name='Conservative Strategy', 
