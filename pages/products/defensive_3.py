...
# --- Marketing Section ---
st.title("🛡️ Global Moderate Growth")
st.markdown("<h3 style='color: #94a3b8; font-weight: 400; margin-top: -10px;'>Optimized Risk-Return Balance</h3>", unsafe_allow_html=True)
...
with col_m1:
    ...
    <div style="padding-right: 20px;">
        <p>
            The <b>Global Moderate Growth</b> strategy targets investors seeking capital appreciation with a controlled risk profile. 
            By applying a moderate risk aversion parameter (Utility Maximization), it captures more upside potential than purely defensive strategies while maintaining strict volatility controls.
        </p>
        <p>
            This strategy is the ideal middle ground, offering a robust participation in equity markets with a systematic mechanism to dampen severe drawdowns.
        </p>
        <ul style="list-style-type: none; padding-left: 0;">
            <li style="margin-bottom: 10px;">✅ <b>Balanced Exposure</b>: Higher equity beta than MinVol for better growth capture.</li>
            <li style="margin-bottom: 10px;">✅ <b>Dynamic Allocation</b>: Shifts towards stability during high volatility regimes.</li>
            <li>✅ <b>Long-Term Compounder</b>: Designed for consistent performance over full market cycles.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    
    st.info("💡 **Investment Profile**: Moderate investors willing to accept some fluctuation for higher long-term returns. Horizon: 5+ Years.")

with col_m2:
    st.markdown("""
    <div class="marketing-card">
        <h4>Key Characteristics</h4>
        <div class="feature-item"><span class="feature-icon">⚖️</span> Utility Maximization</div>
        <div class="feature-item"><span class="feature-icon">📊</span> Moderate Risk Aversion</div>
        <div class="feature-item"><span class="feature-icon">🔄</span> Monthly Rebalancing</div>
        <div class="feature-item"><span class="feature-icon">🚀</span> Growth Oriented</div>
    </div>
    """, unsafe_allow_html=True)
...
# --- Action Section ---
if 'show_results_defensive_3' not in st.session_state:
    st.session_state.show_results_defensive_3 = False

col_btn1, col_btn2, col_btn3 = st.columns([1, 2, 1])
with col_btn2:
    if st.button("🚀 Simulate Historical Performance (2010-2024)", use_container_width=True, type="primary"):
        st.session_state.show_results_defensive_3 = True

if st.session_state.show_results_defensive_3:
    st.markdown("---")
    
    # Run Strategy (Cached in Session State)
    if 'strategy_results_moderate' not in st.session_state:
        with st.spinner("🔄 Optimizing Portfolio & Running Backtest..."):
            st.session_state.strategy_results_moderate = run_strategy("Defensive Moderate")
    
    results = st.session_state.strategy_results_moderate
    ...
    with tab1:
        ...
        fig.add_trace(go.Scatter(x=cum_ret_series.index, y=cum_ret_series, name='Moderate Strategy', 
                                 line=dict(color='#38bdf8', width=2)), row=1, col=1) # Sky-400
    ...
    with tab3:
        st.markdown("#### Strategy Specifications")
        
        col_d1, col_d2 = st.columns(2)
        with col_d1:
             st.markdown("""
            <div class="marketing-card">
                <p><b>Objective:</b> Maximize Utility (Risk Aversion λ=7).</p>
                <p><b>Rebalancing Frequency:</b> Monthly.</p>
                <p><b>Universe:</b> 50 Random Assets (US Large Cap Proxy).</p>
                <p><b>Constraints:</b> Long Only (No Short Selling), Fully Invested.</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col_d2:
            st.markdown("""
            <div class="marketing-card">
                <p><b>Best Month:</b> {:.2%}</p>
                <p><b>Worst Month:</b> {:.2%}</p>
                <p><b>Positive Months (Win Rate):</b> {:.1%}</p>
            </div>
            """.format(best_m, worst_m, win_rate), unsafe_allow_html=True)
    ...
    with tab4:
        ...
        with col_sim1:
            ...
            <div style="margin-top: 20px; padding: 15px; background: rgba(59, 130, 246, 0.1); border-radius: 8px; border: 1px solid #3b82f6;">
                <h5 style="color: #3b82f6; margin:0;">Historical Result (2010-2024)</h5>
                <h2 style="color: white; margin: 5px 0;">${hist_final:,.0f}</h2>
                <p style="color: #10b981; margin:0;">+{hist_profit:,.0f} (+{hist_multiple*100-100:.0f}%)</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col_sim2:
            ...
            for path in paths[:50]: # Show first 50 paths
                fig_mc.add_trace(go.Scatter(x=time_axis, y=path, mode='lines', 
                                          line=dict(color='rgba(56, 189, 248, 0.1)', width=1), showlegend=False))
            ...
            <div style="text-align: center; margin-top: 20px; padding: 20px; border-top: 1px solid #334155;">
                ...
                <h1 style="color: #10b981; font-size: 3rem; font-weight: 800; margin: 0;">${avg_final:,.0f}</h1>
